...
func TestGenerateMountBindings(t *testing.T) {
	...
	if len(result) != len(expectedResult) {
		t.Errorf("expected %d mounts, got %d", len(expectedResult), len(result))
	}
	for i := 0; i < len(result) && i < len(expectedResult); i++ {
		if result[i] != expectedResult[i] {
			t.Errorf("expected mount binding: %s, got: %s", expectedResult[i], result[i])
		}
	}
}
