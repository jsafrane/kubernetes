...
func (og *operationGenerator) GenerateMountVolumeFunc(
	...
	return func() error {
		if volumeAttacher != nil {
			...
			_, resizeError := og.checkForResize(volumeToMount, devicePath, mounter)
...
func (og *operationGenerator) checkForResize(volumeToMount VolumeToMount, devicePath string, mounter mount.Interface) (bool, error) {
	...
	if expandableVolumePlugin != nil &&
		...
		if err != nil {
			...
			return false, volumeToMount.GenerateErrorDetailed("MountVolume get PVC failed", err)
		}
		...
		if pvcStatusCap.Value() < pvSpecCap.Value() {
			...
			if resizeErr != nil {
				...
				return false, resizeDetailedError
			}
			...
			if err != nil {
				...
				return true, volumeToMount.GenerateErrorDetailed("MountVolume update PVC status failed", err)
			}
			return true, nil
		}
	}
