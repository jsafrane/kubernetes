	...
	if cnt == 0 {
		...
		if err == nil && refCount == 0 {
			...
			if found {
				...
				out, err := c.exec.Run("iscsiadm", "-m", "node", "-p", portal, "-T", iqn, "-I", iface, "--logout")
				if err != nil {
				...
				out, err = c.exec.Run("iscsiadm", "-m", "node", "-p", portal, "-T", iqn, "-I", iface, "-o", "delete")
				if err != nil {
					glog.Errorf("iscsi: failed to delete node record Error: %s", string(out))
				}
			} else {
				glog.Infof("iscsi: log out target %s iqn %s", portal, iqn)
				out, err := c.exec.Run("iscsiadm", "-m", "node", "-p", portal, "-T", iqn, "--logout")
				if err != nil {
					glog.Errorf("iscsi: failed to detach disk Error: %s", string(out))
				}
				// Delete the node record
				glog.Infof("iscsi: delete node record target %s iqn %s", portal, iqn)
				out, err = c.exec.Run("iscsiadm", "-m", "node", "-p", portal, "-T", iqn, "-o", "delete")
				if err != nil {
					glog.Errorf("iscsi: failed to delete node record Error: %s", string(out))
				}
			}
