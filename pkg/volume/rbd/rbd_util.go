...
func (util *RBDUtil) rbdLock(b rbdMounter, lock bool) error {
	...
	for i := start; i < start+l; i++ {
		...
		args := append([]string{"lock", "list", b.Image, "--pool", b.Pool, "--id", b.Id, "-m", mon}, secret_opt...)
		...
		if lock {
			...
			if statusErr == nil && !used {
				...
				for _, v := range locks {
					if len(v) > 0 {
						...
						if len(lockInfo) > 2 {
							args := append([]string{"lock", "remove", b.Image, lockInfo[1], lockInfo[0], "--pool", b.Pool, "--id", b.Id, "-m", mon}, secret_opt...)
							cmd, err = b.exec.Run("rbd", args...)
			...
			args := append([]string{"lock", "add", b.Image, lock_id, "--pool", b.Pool, "--id", b.Id, "-m", mon}, secret_opt...)
			cmd, err = b.exec.Run("rbd", args...)
		} else {
...
func (util *RBDUtil) AttachDisk(b rbdMounter) error {
	...
	if !found {
		// modprobe on the host
		exe := exec.New()
		_, err = exe.Command("modprobe", "rbd").CombinedOutput()
		if err != nil {
...
func (util *RBDUtil) CreateImage(p *rbdVolumeProvisioner) (r *v1.RBDVolumeSource, size int, err error) {
	...
	for i := start; i < start+l; i++ {
		...
		output, err = p.rbdMounter.exec.Run("rbd",
			"create", p.rbdMounter.Image, "--size", volSz, "--pool", p.rbdMounter.Pool, "--id", p.rbdMounter.adminId, "-m", mon, "--key="+p.rbdMounter.adminSecret, "--image-format", "1")
		if err == nil {
			break
		} else {
