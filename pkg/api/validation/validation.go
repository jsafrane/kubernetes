...
func validateMountPropagation(mountPropagation *api.MountPropagationMode, container *api.Container, fldPath *field.Path) field.ErrorList {
	...
	if !utilfeature.DefaultFeatureGate.Enabled(features.MountPropagation) {
		allErrs = append(allErrs, field.Invalid(fldPath, mountPropagation, "mount propagation is disabled by feature-gate"))
		return allErrs
	}
	...
	if *mountPropagation == api.MountPropagationBidirectional && !privileged {
		allErrs = append(allErrs, field.Invalid(fldPath, *mountPropagation, "Bidirectional mount propagation is available only to privileged containers"))
	}
	return allErrs
}
